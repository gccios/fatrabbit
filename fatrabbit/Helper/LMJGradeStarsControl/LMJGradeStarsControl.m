...
- (void)buildStars{
    ...
    for (int index = LMJGradeStarsControlStartIndex; index < (LMJGradeStarsControlStartIndex +_totalStars); index++) {
        ...
        if (index <= _defaultIndex) {
            [starBtn setImage:[UIImage imageNamed:@"xinxinyes"] forState:UIControlStateNormal];
            [starBtn setImage:[UIImage imageNamed:@"xinxinyes"] forState:UIControlStateHighlighted];
        }else{
            [starBtn setImage:[UIImage imageNamed:@"xinxinno"] forState:UIControlStateNormal];
            [starBtn setImage:[UIImage imageNamed:@"xinxinno"] forState:UIControlStateHighlighted];
        }
...
- (void)clickStarBtnActionWithBtnTag:(NSInteger)tag{
    for (int i = 0; i < _starsBtnArr.count; i++) {
        ...
        if (btn.tag <= tag) {
            [btn setImage:[UIImage imageNamed:@"xinxinyes"] forState:UIControlStateNormal];
            [btn setImage:[UIImage imageNamed:@"xinxinyes"] forState:UIControlStateHighlighted];;
        }else{
            [btn setImage:[UIImage imageNamed:@"xinxinno"] forState:UIControlStateNormal];
            [btn setImage:[UIImage imageNamed:@"xinxinno"] forState:UIControlStateHighlighted];
        }
    }
